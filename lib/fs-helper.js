  ...
  tilesInsideFolder: function( dirBase, cb ){
    ...
    fs.readdir( dirBase, function( err, files ) {
      cb( err, files.filter( function( file ){
        ...
        return fs.lstatSync( destinationURL ).isDirectory() &&
               fs.existsSync( path.join( destinationURL, 'package.json' ) ) && // I Know, I am lazy sorry
               fs.existsSync( path.join( destinationURL, 'client', 'index.html' ) )
      } ) );
    } );
  }
};
