...
MountManager.prototype.mountTileByFolder = function( locationURL, cb ){
  ...
  installOperation.on( 'close', function( code ){
    ...
    self._app.use( '/tiles/' + appPkg.name, require( path.join( locationURL, 'server' ) ) );
 ...
 * @param {Function} cb, callback
 * */
MountManager.prototype.mountTileByGitURL = function( locationGitURL, settings, cb ){
	var self = this,
			locationGitURL 	= locationGitURL.replace('git@github.com:','https://github.com/'),
    ...
    debug( 'Cloning tile %s from url %s', body.name, rawPkgURL );
		...
		git.clone( locationGitURL, destinationURL, function(err, repo) {
			...
			rm(path.join( destinationURL, '.git'), function() {
				mkpath.sync( path.join( destinationURL, 'server', 'config' ), 0700 );
				fs.writeFileSync( path.join( destinationURL, 'server', 'config', 'settings.json' ), settings );
